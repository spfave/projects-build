# **Go Server**
The Go directory is  
<!-- https://go.dev/doc/modules/layout -->

&nbsp;
## **Directory structure**
- The `cmd` directory
- The `internal` directory
- The `pkg` directory

### **Apps**
- json API
- A json API application using [Go](https://go.dev/) and its standard library
<!-- - Multi-page web application with HTMX for client side interactions -->


&nbsp;
## **References**
[Go Docs](https://go.dev/doc/)  
[Go Blog: Error Handling and Go](https://go.dev/blog/error-handling-and-go)  
[Go Blog: JSON and Go](https://go.dev/blog/json)  

[Alex Edwards: Lets Go](https://lets-go.alexedwards.net/)  
[Alex Edwards: Lets Go Further](https://lets-go-further.alexedwards.net/)  
[HTTP Services in Go - Mat Ryer](https://grafana.com/blog/2024/02/09/how-i-write-http-services-in-go-after-13-years/)  
[HTTP Services in Go - Bjorn V](https://dev.to/omg-a-bear/how-i-write-http-services-in-go-2k34)
[Http Routing with Go Standard Library](https://github.com/dreamsofcode-io/nethttp)  
[Go Routing: Handling and Grouping Routes](https://dev.to/kengowada/go-routing-101-handling-and-grouping-routes-with-nethttp-4k0e)  
[Go debug with Air](https://www.arhea.net/posts/2023-08-25-golang-debugging-with-air-and-vscode/)  
[Air and Debugging](https://dev.to/nerdherd/setting-up-golang-on-vs-code-with-debugging-1kbe)  

[Example: Go Api - Gopher Social](https://github.com/sikozonpc/GopherSocial)

